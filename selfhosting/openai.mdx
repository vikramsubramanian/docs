...
We need to select a region based on availability of OpenAI models. East US is by far the most available regions. See [here](https://learn.microsoft.com/en-us/azure/ai-services/openai/quotas-limits)
...
Various details generated during the setup process above will need to be copied and provided to Mayil's backend API as configuation details. Update the JSON block below with all the procured information. 
...
If the variables names provided in the commands in the previous step were used as is, few modifications need to be made to the config.
...
4. `key` - A identifier used by Mayil. Can be one of ["GPT4O", "EMBEDDING", "GPT3"]
